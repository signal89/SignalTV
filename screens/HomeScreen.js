...
export default function HomeScreen({ navigation }) {
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity style={styles.gridItem} onPress={() => navigation.navigate("Category", { category: item, tab: activeTab })}>
  ...
  return (
    <View style={styles.container}>
      ...
      {loading ? (
        ...
        <FlatList data={categories} renderItem={renderItem} keyExtractor={(item) => item} numColumns={3} contentContainerStyle={{ padding: 10 }} />
      ) : (
        <Text style={{ color: "white", textAlign: "center", marginTop: 20 }}>Nema dostupnih kategorija.</Text>
      )}
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  gridItem: { backgroundColor: "#007AFF", height: itemSize, width: itemSize, margin: 5, justifyContent: "center", alignItems: "center", borderRadius: 10 },
